...
Page Store Format
	...
	| number of available pages | pointer to next page store |  pointers to available pages   |
...
func (store *PageStore) getNext() PagePointer {
	return PagePointer(binary.LittleEndian.Uint64(store.data[2:]))
}

func (store *PageStore) setNext(pointer PagePointer) {
